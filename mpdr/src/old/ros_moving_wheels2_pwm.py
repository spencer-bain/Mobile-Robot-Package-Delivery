...
class MotorDriver:
    ...
    def twist_callback(self,data):
        ...
        print("Raw initial caluculatved velocity left wheel: ",left_speed_out)
        print("Raw initial caluculatved velocity right wheel: ",right_speed_out)
        ...
        #make sure that the linear2pwm function takes care of the "deadzone".
        #Meaning that right now the lowerbound of the linear is 0.159(meter/sec) which is 
        #Mapped to a 6% duty cylce. The two line below takes care of if we send a speed
        #less than 0.159(meter/sec), because we can't reach those speeds with our current
        #implimentation
        if abs(left_speed_out) < linear_vel_bounds[3] and abs(left_speed_out > linear_vel_bounds[1]):
            left_speed_out = 0
        if abs(right_speed_out) < linear_vel_bounds[3] and abs(right_speed_out > linear_vel_bounds[1]):
            right_speed_out = 0
        ...
        print("velocity passed to set_vel_L",left_speed_out)
        print("velocity passed to set_vel_R",right_speed_out)
        info = self.status()
